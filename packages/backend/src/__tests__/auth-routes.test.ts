...
import {
  ...
  initTestAuth,
} from "./test-auth-helper.js";
import { _resetSyncCache } from "../services/user-sync.js";
...
describe("Auth Routes (Hub Auth)", () => {
  ...
  describe("GET /api/auth/me", () => {
    ...
    it("creates app_users record on first visit (lazy sync)", async () => {
      ...
      const row = db
        .prepare("SELECT * FROM app_users WHERE hub_user_id = ?")
        .get("42") as {
        hub_user_id: string;
        display_name: string;
        email: string;
    ...
    it("updates app_users on subsequent visits", async () => {
      ...
      // Reset throttle so the second sync runs immediately
      _resetSyncCache();
