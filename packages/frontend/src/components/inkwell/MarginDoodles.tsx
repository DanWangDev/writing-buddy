...
/** Game-inspired margin doodles at low opacity — Minecraft blocks, Genshin gems, Among Us beans, Fortnite loot */
export function MarginDoodles({ className = '', variant = 'default' }: Props) {
  return (
    <div className={`pointer-events-none select-none ${className}`} aria-hidden="true">
      {/* Top-right: blocky world elements */}
      <svg
        className="absolute top-3 right-4 w-16 h-16 sm:w-20 sm:h-20 opacity-[0.08]"
        viewBox="0 0 80 80"
        fill="none"
      >
      ...
      <svg
        className="absolute bottom-4 left-3 w-14 h-14 sm:w-18 sm:h-18 opacity-[0.08]"
        viewBox="0 0 70 60"
        fill="none"
      >
        <g stroke="#44403C" strokeWidth="1.5" fill="none" strokeLinecap="round">
      ...
      {/* Mid-right: primogem + elemental orb */}
      <svg
        className="absolute top-1/2 right-2 -translate-y-1/2 w-12 h-14 sm:w-14 sm:h-16 opacity-[0.07]"
        viewBox="0 0 50 60"
        fill="none"
      >
      ...
      {(variant === 'portal' || variant === 'prompts') && (
        <svg
          className="absolute top-5 left-5 w-12 h-14 opacity-[0.07]"
          viewBox="0 0 50 55"
          fill="none"
        >
          <g stroke="#44403C" strokeWidth="1.5" fill="none" strokeLinecap="round">
      ...
      {/* Bottom-right: constellation dots */}
      {variant !== 'writing' && (
        <svg
          className="absolute bottom-6 right-8 w-16 h-12 opacity-[0.06]"
          viewBox="0 0 60 45"
          fill="none"
        >
          <g fill="none">
